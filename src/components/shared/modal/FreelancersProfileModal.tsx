    ...
    <div className=" w-full flex items-center justify-between mb-[30px] lg:mb-[42px]">
      ...
      >
        <Link
          ...
          target="_blank"
          className="w-full flex items-center text-[#005AFF]    text-[18px] md:text-[20px] lg:text-[22px] leading-[25px] lg:leading-[40px]"
        >
...
const FreelancersProfileModal = () => {
  ...
  useEffect(() => {
    if (active) {
      console.log(active, "its active now");
  ...
  if (active)
    return (
      <div
        onClick={(e) => {
          console.log("clicked freelancers profile");
          e.stopPropagation();
          dispatch(setActiveFreelancerProfileModal(false));
          document.body.style.overflowY = "auto";
        }}
